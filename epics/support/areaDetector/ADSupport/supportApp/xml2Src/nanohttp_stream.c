...
xmlNanoHTTPStreamInit(void) {
	...
	if (env != NULL) {
	    xmlNanoHTTPStreamScanProxy(env);
	    goto done;
	}
	env = getenv("HTTP_PROXY");
	if (env != NULL) {
...
void*
xmlNanoHTTPStreamOpen(const char *URL, char **contentType) {
    if (contentType != NULL) *contentType = NULL;
    return(xmlNanoHTTPStreamMethod(URL, NULL, NULL, contentType, NULL, 0));
}
...
void*
xmlNanoHTTPStreamOpenRedir(const char *URL, char **contentType, char **redir) {
    if (contentType != NULL) *contentType = NULL;
    if (redir != NULL) *redir = NULL;
    return(xmlNanoHTTPStreamMethodRedir(URL, NULL, NULL, contentType, redir, NULL,0));
}
 ...
 */
int
xmlNanoHTTPStreamRead(void *ctx, void *dest, int len) {
    xmlNanoHTTPCtxtPtr ctxt = (xmlNanoHTTPCtxtPtr) ctx;
#ifdef HAVE_ZLIB_H
    int bytes_read = 0;
    int orig_avail_in;
...
void*
xmlNanoHTTPStreamMethodRedir(const char *URL, const char *method, const char *input,
...
void*
xmlNanoHTTPStreamMethod(const char *URL, const char *method, const char *input,
                  char **contentType, const char *headers, int ilen) {
    return(xmlNanoHTTPStreamMethodRedir(URL, method, input, contentType,
		                  NULL, headers, ilen));
}
 ...
 */
int
xmlNanoHTTPStreamFetch(const char *URL, const char *filename, char **contentType) {
    void *ctxt = NULL;
    char *buf = NULL;
    int fd;
    int len;
    ...
    ctxt = xmlNanoHTTPStreamOpen(URL, contentType);
 ...
 */
int
xmlNanoHTTPStreamSave(void *ctxt, const char *filename) {
    char *buf = NULL;
    int fd;
    int len;
    int ret = 0;
